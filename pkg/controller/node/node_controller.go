...
func (r *ReconcileNode) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	_ = log.WithValues("rr-operator", request.NamespacedName)
	...
	if yes, err := r.isRrEnabled(); err != nil {
		...
		log.Info("rr-operator", "====================================================================", "")
		log.Info("rr-operator", "Calico NodeToNodeMesh is ENABLED, starting migration to RRs workflow", "")
		log.Info("rr-operator", "====================================================================", "")
...
func (r *ReconcileNode) selectRrNodes(nodes *corev1.NodeList) ([]*corev1.Node, error) {
	...
	for i := range nodes.Items {
		...
		if rrPerZone[nodes.Items[i].GetLabels()[zoneLabel]] < numberOfRRsPerZone {
			log.Info("rr-operator", "Selecting node for RR", nodes.Items[i].GetName())
